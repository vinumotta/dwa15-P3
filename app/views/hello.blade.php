...
<body>
	<div>
		...
		if ($array['returned_paragraphs'])
		{
			$returned_paragraphs = $array['returned_paragraphs'];
		}
		
		
		echo Form::open(array('url' => '/', 'method' => 'get'));
		echo Form::text('paragraphs');
		echo Form::submit('Submit!');
		echo Form::close();

		/*
		if ($array['returned_users'])
		{
		...
		*/
	    ...
	    <input type='submit' value='Generate paragraphs!' name='paragraph_submit'><br>
		</form>

		<form method='get' name='users_form' action='/users'>
		Enter User Parameters<br>
		...
		{
			$returned_paragraphs = $array['returned_paragraphs'];
			for ($i = 0; $i < count($returned_paragraphs); $i++)
			{
				echo $returned_paragraphs[$i] . '</br>' . $i. count($returned_paragraphs);
			}
		}
		//echo implode('<p>', $returned_paragraphs);?></p2>
	</div>
</body>
</html>
